  ...
  return (
    <form onSubmit={handleSubmit}>
      <Box style={{ backgroundImage: "linear-gradient(to left bottom, #fc0839, #fc0742, #fb094a, #fa0e52, #f8145a)" }} maxWidth={400} display="flex" flexDirection="column" alignItems="center" justifyContent="center" margin="auto" marginTop={5} boxShadow="10px 10px 20px #ccc" padding={3} borderRadius={5}>
        <Typography variant="h4" sx={{ textTransform: "uppercase" }} padding={3} textAlign="center" color="white">Register</Typography>
        <TextField placeholder="name" value={inputs.name} onChange={handleChange} name="name" margin="normal" type="text" required sx={{ input: { backgroundColor: "white", borderRadius: "15px" } }} />
        ...
        <Button onClick={() => navigate("/login")} sx={{ borderRadius: 3, marginTop: 3 }}>
          <Typography variant="body2" sx={{ textTransform: "uppercase" }} padding={1} textAlign="center" color="white">Already registered? Please Login</Typography>
        </Button>
      </Box>
    </form>
  );
